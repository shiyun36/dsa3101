...
from text_extraction import extract_esg_text
from rag_processing import process_rag
...
def main():
    ...
    if conn:
        ...
        esg_text_df = extract_esg_text(args.url, args.country, args.industry)
        if esg_text_df.empty:
            print("No ESG text extracted. Skipping processing.")
        else:
            # Step 2: Process RAG
            rag_df = process_rag(esg_text_df)
